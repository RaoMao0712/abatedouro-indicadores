...
def gerar_pdf(rc):
    b=BytesIO(); doc=SimpleDocTemplate(b,pagesize=A4,leftMargin=14*mm,rightMargin=14*mm,topMargin=30*mm,bottomMargin=18*mm,title=rc["numero"]); s=getSampleStyleSheet(); story=[Paragraph(f"<b>{escape(rc['numero'])}</b> — {escape(rc['status'])}",s["Heading2"]),Spacer(1,4*mm)]
